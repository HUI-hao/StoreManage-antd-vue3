...
export default {
  ...
  created() {
    this.$axios.post(this.$baseurl + '/user/alluser').then(response => {
  ...
  methods:
      {
        onEdit(value) {
          ...
          this.$router.push({path: 'UpdateUser', query: {id: JSON.stringify(value.id), username: pushed}})
        },
        onDelete(value) {
          this.deleteId = {
            "id": JSON.parse(JSON.stringify(value.id))
          }
          this.$axios.post(this.$baseurl + "/user/delete", this.deleteId)
